    ...
    func searchUsers(query: String) async -> [ChatUser] {
        ...
        do {
            ...
            if query.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                return allUsers.filter { $0.id != Auth.auth().currentUser?.uid }
            } else {
            ...
            if let otherUserId = participants.first(where: { $0 != currentUserId }) {
                do {
                    ...
                    if let data = doc.data() {
                        ...
                        let avatarUrl = data["photoURL"] as? String
                        let university = data["university"] as? String
                        enrichedConversations[i].otherUser = ChatUser(id: otherUserId, name: name, avatarUrl: avatarUrl, university: university)
                    } else {
                        enrichedConversations[i].otherUser = ChatUser(id: otherUserId, name: "Usuario", avatarUrl: nil, university: nil)
                    }
                } catch {
                    enrichedConversations[i].otherUser = ChatUser(id: otherUserId, name: "Usuario", avatarUrl: nil, university: nil)
                }
